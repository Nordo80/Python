...
def find_smallest_animal_by_weight(animal_list: list) -> Animal:
    ...
    """
    return sorted(animal_list, key=lambda x: x.weight_range, reverse=False)[0]


def animal_names(s):
    """Moego pesika zovut Bonnie, ljublju ego."""
    return s.species, s.scientific_name
...
def list_species_and_scientific_names(animal_list: list) -> list:
    ...
    """
    return list(map(animal_names, animal_list))


def pumpkin_lilia(s):
    """Privet lilja <3."""
    return (max(s.weight_range) + min(s.weight_range)) * 0.06 / 3
...
def find_how_many_pumpkins_are_needed_to_feed_animals(animal_list: list) -> int:
    ...
    """
    return math.ceil(functools.reduce(lambda a, b: a + b, list(
        map(pumpkin_lilia, list(filter(lambda x: x.diet != "carnivorous", animal_list)) * 90))))
...
def to_animal_description(s):
    """We need to print out animal descriptions to display near the animals at our zoo."""
    return f"{s.species} ({s.scientific_name}) lives in {s.habitat} and its diet is {s.diet}. These animals can live up to {s.age_up_to} years and they weigh between {min(s.weight_range)} kg and {max(s.weight_range)} kg as adults."
...
def create_animal_descriptions(animal_list: list) -> list:
    ...
    """
    return list(map(to_animal_description, animal_list))
